...
public class WebLogAdminAspect {
  ...
  /** 雪花算法 1 : 机器的id 2 : 应用的id */
  private Snowflake snowflake = new Snowflake(1, 1);

  @Autowired private SysUserLogService sysUserLogService;
  /** 日志记录： 环绕通知：方法执行之前、之后 */

  /** 1 定义切入点 */
  @Pointcut("execution(* com.levtea.controller.*.*(..))") // controller 包里面所有类，类里面的所有方法 都有该切面
  public void webLog() {}
  ...
  public Object recodeWebLog(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
    ...
    webLog.setUsername(
        authentication == null ? "anonymous" : authentication.getPrincipal().toString()); // 获取用户的id
