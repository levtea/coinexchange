...
import com.levtea.vo.UserAuthInfoVo;
...
public class UserController implements UserServiceFeign {
  ...
  @ApiImplicitParams({@ApiImplicitParam(name = "id", value = "会员ID")})
  ...
  public R<Page<User>> findUserAuths(
    ...
    Page<User> userPage =
        userService.findByPage(page, mobile, userId, userName, realName, null, reviewsStatus);
    return R.ok(userPage);
  }
    ...
    if (user != null) {
      ...
      if (reviewsStatus == null || reviewsStatus == 0) { // 待审核
        ...
        UserAuthAuditRecord userAuthAuditRecord =
            userAuthAuditRecordList.get(0); // 之前我们查询时,是按照认证的日志排序的,第0 个值,就是最近被认证的一个值
  ...
  @ApiImplicitParams({@ApiImplicitParam(name = "userAuthForm", value = "userAuthFormjson数据")})
  ...
  @ApiImplicitParams({@ApiImplicitParam(name = "imgs", value = "用户的图片地址")})
  ...
  @ApiImplicitParams({@ApiImplicitParam(name = "updateLoginParam", value = "修改用户的登录密码")})
  ...
  @ApiImplicitParams({@ApiImplicitParam(name = "updateLoginParam", value = "修改用户的交易密码")})
