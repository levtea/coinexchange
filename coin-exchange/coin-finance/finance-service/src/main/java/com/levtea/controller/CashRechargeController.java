...
import com.levtea.util.ReportCsvUtils;
...
public class CashRechargeController {
  ...
  @Autowired private CashRechargeService cashRechargeService;
  ...
  public R<Page<CashRecharge>> findByPage(
      ...
      String startTime,
      String endTime) {
    Page<CashRecharge> pageData =
        cashRechargeService.findByPage(
            page, coinId, userId, userName, mobile, status, numMin, numMax, startTime, endTime);
  ...
  public void recordsExport(
    ...
    if (!CollectionUtils.isEmpty(records)) {
      ...
      CellProcessorAdaptor longToStringAdapter =
      ...
      CellProcessorAdaptor moneyCellProcessorAdaptor =
          new CellProcessorAdaptor() {
            @Override
            public <T> T execute(Object o, CsvContext csvContext) {
      ...
      CellProcessorAdaptor typeAdapter =
          new CellProcessorAdaptor() {
            @Override
            public <T> T execute(Object o, CsvContext csvContext) {
      ...
      CellProcessorAdaptor timeCellProcessorAdaptor =
          new CellProcessorAdaptor() {
            @Override
            public <T> T execute(Object o, CsvContext csvContext) {
              if (o == null) {
      ...
      CellProcessorAdaptor statusCellProcessorAdaptor =
          new CellProcessorAdaptor() {
            @Override
            public <T> T execute(Object o, CsvContext csvContext) {
      ...
      CellProcessor[] PROCESSOR =
          new CellProcessor[] {
            ...
            null,
            null,
            null, // "ID", "用户ID", "用户名", "真实用户名", "充值币种",
            moneyCellProcessorAdaptor,
            moneyCellProcessorAdaptor,
            moneyCellProcessorAdaptor,
            typeAdapter, // "充值金额(USDT)", "手续费", "到账金额(CNY)", 充值方式"
            null,
            null,
            timeCellProcessorAdaptor,
            timeCellProcessorAdaptor, // 充值订单", "参考号", "充值时间", "完成时间
  ...
  public R<Page<CashRecharge>> findUserCashRecharge(
      @ApiIgnore Page<CashRecharge> page, Byte status) {
    Long userId =
        Long.valueOf(
            SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
    Page<CashRecharge> cashRechargePage =
        cashRechargeService.findUserCashRecharge(page, userId, status);
    return R.ok(cashRechargePage);
  }

  @ApiOperation(value = "现金的充值审核")
  @ApiImplicitParams({@ApiImplicitParam(name = "cashRechargeAuditRecord", value = "现金的充值审核")})
