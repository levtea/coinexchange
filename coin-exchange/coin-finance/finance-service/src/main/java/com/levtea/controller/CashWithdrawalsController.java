...
public class CashWithdrawalsController {
  @Autowired private CashWithdrawalsService cashWithdrawalsService;
  ...
  public R<Page<CashWithdrawals>> findByPage(
    ...
    Page<CashWithdrawals> pageData =
        cashWithdrawalsService.findByPage(
            page, userId, userName, mobile, status, numMin, numMax, startTime, endTime);
    ...
    if (!CollectionUtils.isEmpty(records)) {
      ...
      CellProcessor[] cellProcessors =
          new CellProcessor[] {
            ...
            moneyCellProcessorAdaptor,
            moneyCellProcessorAdaptor, // "提现金额(USDT)", "手续费", "到账金额",
            null,
            null,
            null, // "提现开户名", "银行名称, 账号"
            timeCellProcessorAdaptor,
            timeCellProcessorAdaptor, //  "申请时间", "完成时间"
  ...
  public R<Page<CashWithdrawals>> findUserCashWithdrawals(
      @ApiIgnore Page<CashWithdrawals> page, Byte status) {
    Long userId =
        Long.valueOf(
            SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
    Page<CashWithdrawals> cashWithdrawalsPage =
        cashWithdrawalsService.findCashWithdrawals(page, userId, status);
