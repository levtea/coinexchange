...
import com.levtea.mapper.CoinRechargeMapper;
import com.levtea.service.CoinRechargeService;
...
public class CoinRechargeServiceImpl extends ServiceImpl<CoinRechargeMapper, CoinRecharge>
  ...
  @Override
  public Page<CoinRecharge> findByPage(
    ...
    if (userId != null
      ...
      coinRechargeLambdaQueryWrapper.in(
          !CollectionUtils.isEmpty(userIds), CoinRecharge::getUserId, userIds);
    }
    // 2 若用户本次的查询中,没有带了用户的信息
    coinRechargeLambdaQueryWrapper
    ...
    if (!CollectionUtils.isEmpty(records)) {
      List<Long> userIds =
          records.stream().map(CoinRecharge::getUserId).collect(Collectors.toList());
      if (CollectionUtils.isEmpty(basicUsers)) {
        basicUsers = userServiceFeign.getBasicUsers(userIds, null, null);
      }
      Map<Long, UserDto> finalBasicUsers = basicUsers;
      records.forEach(
          coinRecharge -> { // 设置用户相关的数据
