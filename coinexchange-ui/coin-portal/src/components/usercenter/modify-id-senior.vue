<template>
  <div
    class="ucenter-wrap"
    v-wechat-title="$t('m.userCenter.advancedCertification.title')"
  >
      ...
      <div class="ucenter-row">
        ...
        <div class="ucenter-main">
          ...
          <div class="ucenter-body">
            <el-alert
              type="warning"
              :title="$t('m.userCenter.advancedCertification.uploadSizeLimit')"
            ></el-alert>
            <el-form ref="ruleForm" :model="ruleForm" :rules="rules">
              <div class="item">
                <div class="item-title">
                  {{ $t("m.userCenter.advancedCertification.cardFrontTips") }}
                </div>
                <el-row :gutter="24">
                  <el-col :span="10">
                    <el-form-item prop="imageUrl1">
                      ...
                      >
                        <img
                          v-if="ruleForm.imageUrl1"
                          :src="ruleForm.imageUrl1"
                          class="avatar"
                        />
                  ...
                  <el-col :span="4" class="eg">
                    {{ $t("m.userCenter.advancedCertification.example")
              ...
              <div class="item">
                <div class="item-title">
                  {{ $t("m.userCenter.advancedCertification.cardBackTips") }}
                </div>
                <el-row :gutter="24">
                  <el-col :span="10">
                    <el-form-item prop="imageUrl2">
                      ...
                      >
                        <img
                          v-if="ruleForm.imageUrl2"
                          :src="ruleForm.imageUrl2"
                          class="avatar"
                        />
                        <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                      </el-upload>
              ...
              <div class="item">
                <div class="item-title">
                  {{
                    $t(
                      "m.userCenter.advancedCertification.frontPhotoAndSignatureTips"
                    )
                  }}
                  <p class="description">
                    {{
                      $t(
                        "m.userCenter.advancedCertification.frontPhotoAndSignatureDesc"
                      )
                    }}
                  </p>
                </div>
                <el-row :gutter="24">
                  <el-col :span="10">
                    <el-form-item prop="imageUrl3">
                      ...
                      >
                        <img
                          v-if="ruleForm.imageUrl3"
                          :src="ruleForm.imageUrl3"
                          class="avatar"
                        />
                        <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                      </el-upload>
            ...
            validator: (rule, value, callback) => {
              if (value === "") {
                callback(
                  new Error(
                    this.$t(
                      "m.userCenter.advancedCertification.plzUpFrontPhoto"
                    )
                  )
                );
              } else {
        ...
        imageUrl2: [
          {
            validator: (rule, value, callback) => {
              if (value === "") {
                callback(
                  new Error(
                    this.$t("m.userCenter.advancedCertification.plzUpBackPhoto")
                  )
                );
              } else {
                callback();
            ...
            validator: (rule, value, callback) => {
              if (value === "") {
                callback(
                  new Error(
                    this.$t(
                      "m.userCenter.advancedCertification.plzUpHandheldPhoto"
                    )
                  )
                );
              } else {
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let avatar = [
            this.ruleForm.imageUrl1,
            this.ruleForm.imageUrl2,
            this.ruleForm.imageUrl3,
          ];
          console.log(avatar);
      ...
      this.$msgbox({
        ...
        message: h("p", null, [
          ...
          h(
            "div",
            { style: "color: teal" },
            this.$t("m.userCenter.certificationSuccess")
          ),
        ]),
    ...
    handleUploadSuccess1(response) {
      ...
      if (Status === "OK") {
        this.ruleForm.imageUrl1 = uploadApi.aliyunFileUrl + this.uploadData.key;
      }
    },
    // 反面
    handleUploadSuccess2(response) {
      ...
      const { Status } = response;
      if (Status === "OK") {
        this.ruleForm.imageUrl2 = uploadApi.aliyunFileUrl + this.uploadData.key;
      }
    },
    // 手持
    handleUploadSuccess3(response) {
      ...
      const { Status } = response;
      if (Status === "OK") {
        this.ruleForm.imageUrl3 = uploadApi.aliyunFileUrl + this.uploadData.key;
      }
    },
    async beforeUpload() {
      ...
      if (res.data) {
        ...
        let { dir, policy, signature, callback, accessid, host } =
          preUploadData;
